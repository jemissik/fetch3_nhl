...
####################
def calc_LAD(z_Above, dz, z_m, Hspec):
    ...
    for i in np.arange(0,len(z_LAD),1):
        if  0.1<=z_LAD[i]<z_m:
            LAD[i]=cfg.L_m*(((Hspec-z_m)/(Hspec-z_LAD[i]))**6)*np.exp(6*(1-((Hspec-z_m)/(Hspec-z_LAD[i]))))
        if  z_m<=z_LAD[i]<Hspec:
            LAD[i]=cfg.L_m*(((Hspec-z_m)/(Hspec-z_LAD[i]))**0.5)*np.exp(0.5*(1-((Hspec-z_m)/(Hspec-z_LAD[i]))))
        if z_LAD[i]==Hspec:
            LAD[i]=0
    return LAD
LAD = calc_LAD(z_Above, cfg.dz, cfg.z_m, cfg.Hspec)